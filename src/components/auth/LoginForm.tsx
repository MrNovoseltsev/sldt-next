...
} from '@/lib/mock-auth/MockAuthContext'
...
export default function LoginForm() {
  ...
  const {
    ...
    defaultValues: { email: MOCK_LOGIN_EMAIL, password: MOCK_DEFAULT_PASSWORD },
  })

  const onSubmit = (values: LoginValues) => {
    setServerError(null)
    const { error } = signIn(values.email, values.password)
    if (error) {
    ...
    <div className="w-full flex flex-col items-center">
      ...
      <div className="bg-[var(--surface)] border border-border rounded-lg p-[28px_28px_24px] w-full max-w-[360px]">
        ...
        <div className="text-xs text-muted-foreground mb-6">
          Демо-доступ: учётные данные подставлены — нажмите «Войти»
        </div>
